...
?>
<main id="main">
<section class="short-hero d-flex mb-0" style="background-image:url(<?=$img?>);">
    ...
    <div class="container-xl py-5">
        <div class="news__grid">
            ...
            while ($w->have_posts()) {
                $w->the_post();
                ?>
                <a class="news__card" href="<?=get_the_permalink()?>">
                    <img src="<?=get_the_post_thumbnail_url($w->ID, 'large')?>">
                    <div class="news__inner">
                        <h3><?=get_the_title()?></h3>
                        <div class="latest_news__date"><?=get_the_date('j / m / Y')?></div>
                        <div class="latest_news__excerpt"><?=wp_trim_words(get_the_content(null, false, get_the_ID()),30)?></div>
                    </div>
                </a>
                <?php
            }
...
<section class="jobs py-5">
    <div class="container-xl">
        ...
        foreach ($result as $i) {
            ?>
            <a class="jobs__card" href="<?=get_the_permalink($i)?>">
